...
function WaterDilution({ title, intro }: WaterDilutionProps) {
  ...
  return (
    <div>
      {title && <h3>{title}</h3>}
      {intro && <p>{intro}</p>}
      <NumericField label="Densidad actual" name="DO" handleInputChange={(v) => setDO(v)} placeholder="ej: 1052" value={DO} maxLength={4} />
      <NumericField label="Volumen en litros" name="volume" handleInputChange={(v) => setVolume(v)} placeholder="ej: 20" value={volume} maxLength={4} />
      <NumericField label="Densidad objetivo" name="DF" handleInputChange={(v) => setDF(v)} placeholder="ej: 1042" value={DF} maxLength={4} />
